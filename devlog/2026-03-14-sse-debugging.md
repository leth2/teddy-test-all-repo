...
다음으로 `claude-agent-acp` 프로세스가 아예 안 뜨는지 확인했다. `ps aux`로 보니 프로세스는 살아있었다:
```
PID 14 = npm exec @zed-industries/claude-agent-acp
PID 25 = node /root/.npm/.../claude-agent-acp
```

실행은 됐는데 응답이 없다. 그럼 통신 문제다. 새로운 인스턴스로 수동 테스트:
```bash
echo '{"jsonrpc":"2.0","id":0,...}' | node /path/to/claude-agent-acp
# → {"jsonrpc":"2.0","id":0,"result":{...}}  ← 응답 온다!
```

바이너리 자체는 정상 동작한다. 근데 실제 컨테이너에서는 왜 응답이 없을까?

여기서 `/proc` 파일시스템을 꺼냈다. Linux에서 프로세스의 열린 파일 디스크립터를 직접 볼 수 있다:

```bash
ls -la /proc/25/fd/
# fd/0 → socket:[227758]  ← stdin이 파이프가 아님!
# fd/1 → socket:[227760]  ← stdout도 파이프가 아님!
```

`stdio: ['pipe', 'pipe', 'pipe']`로 spawn했는데 실제 프로세스의 stdin이 `pipe`가 아니라 `socket`에 연결돼 있었다. bridge가 쓴 JSON-RPC 메시지가 PID 14(npm exec)에는 전달되지만, PID 14가 PID 25에 stdin을 제대로 포워딩하지 않는 것이다.

이게 "동작하는 것처럼 보이지만 실제로는 아무것도 전달되지 않는" 버그의 정체였다. 에러도 없고, 타임아웃도 아니고, 그냥 조용히 씹히는 것.

---

**QA로서 오늘 배운 것**

버그를 찾는 건 증상에서 시작해 원인까지 파고드는 과정이다. 체크리스트:

1. **증상 확인** — "agent-ready 안 옴"
2. **가설 3개** — 타임아웃? 인증 실패? 프로세스 통신 오류?
3. **수동 재현** — 각 가설을 격리해서 테스트
4. **OS 레벨까지** — 추상화 위에서 안 보이면 그 아래로

코드를 믿지 말고 실행 환경을 확인해야 한다. "spawn에 pipe 옵션 줬으니까 당연히 pipe로 연결되겠지"가 아니라, 실제로 연결됐는지 `/proc`에서 확인해야 한다. 오늘처럼 구조적인 문제일수록, 표면적인 증상만 봐서는 절대 찾을 수 없다.
